...
def assistant(
    assistant_goal="", keep_in_mind="", assistant_identity="", app_name=None, execute_json_case=None, called_from=None
):  # App TestCase Gen
    """
    ...
    if not execute_json_case:
        ...
        assistant_goal = imaging(
            window_title=app_name, additional_context=additional_context, screenshot_size="Full screen"
        ...
        step_creator = [
            ...
            {"role": "system", "content": f'Focused window: "{app_name}"\nGoal: {assistant_goal}'},
        ]
    ...
    if step_analysis:
        ...
        for i, step in enumerate(action_list, start=1):
            ...
            if action == "click_element":
                ...
                time.sleep(1)
                updated_instructions = update_instructions_with_action_string(
                    instructions,
                    act(
                        single_step=f"{step_description}",
                        app_name=app_name,
                        screen_analysis=assistant_goal,
                ...
                database_add_case(
                    database_file, app_name, assistant_goal, updated_instructions
                )  #  Print the entire database with # print_database(database_file)
                ...
                act(
                    single_step=f"{step_description}", double_click=True, app_name=app_name, original_goal=original_goal
                )
                ...
                if len(step_description) < 5:
                    ...
                    if i + 1 < len(action_list) and type(action_list[i + 1]["step"]) == str:
                        ...
                        if i + 1 < len(action_list) and (
                            "press enter" in action_list[i + 1]["step"].lower()
                            or "press the enter" in action_list[i + 1]["step"].lower()
                            or "'enter'" in action_list[i + 1]["step"].lower()
                            or '"enter"' in action_list[i + 1]["step"].lower()
                        ):
                            if urls:
                                for url in urls:
                ...
                while not element_visible and retry_count < max_retries:
                    ...
                    scroll_assistant_goal = check_element_visibility(app_name, step_description)["choices"][0][
                        "message"
                    ]["content"]
                ...
                if debug_step is not True:
                    ...
                    if image_analysis:
                        ...
                        if new_goal:
                            newest_goal = imaging(
                                window_title=app_name, additional_context=additional_context
                            )  # )['choices'][0]['message']['content']
                            # if ": " in newest_goal:
                            ...
                            new_json = api_call(
                                review_output, model_name="gpt-4-1106-preview", max_tokens=4095, temperature=1.0
                            )
                            print("The assistant said:\n", step_analysis)
        ...
        time.sleep(
            calculate_duration_of_speech(
                "Assistant finished the generated test case. Can I help you with something else?"
            )
            / 1000
        )
...
def auto_role(goal):
    assistant_call = [
        {
            "role": "assistant",
            "content": "You are an AI assistant that receives a goal and responds with the best action to perform the goal.\n"
...
def find_element(single_step, app_name, original_goal, avoid_element="", assistant_goal=None, attempt=0):
    ...
    analyzed_ui = analyze_app(
        application_name_contains=app_name, size_category=None, additional_search_options=keywords
    )
    select_element = [
        {
            "role": "assistant",
    ...
    if (
        "sorry" in selected_element.lower()
        or "empty string" in selected_element.lower()
        or "no element" in selected_element.lower()
        or "not found" in selected_element.lower()
        or "no relevant element" in selected_element.lower()
        or "no element found" in selected_element.lower()
    ):
    ...
    if (
        "sorry" in last_coordinates.lower()
        or "empty string" in last_coordinates.lower()
        or "no element" in last_coordinates.lower()
        or "not found" in last_coordinates.lower()
    ):
    ...
    if x == 0 and y == 0 or x == "" and y == "":
        ...
        coordinates = {
            k.strip(): float(v.strip()) for k, v in (item.split("=") for item in last_coordinates.split(","))
        }
        x = coordinates["x"]
        y = coordinates["y"]
...
def act(
    ...
    if rescan_element_match is True:
        ...
        while attempt < max_attempts:
            ...
            coordinates, selected_element, keywords, attempt = find_element(
                single_step, app_name, original_goal, avoid_element, assistant_goal, attempt
            )
            x = coordinates["x"]
            y = coordinates["y"]
            ...
            element_analysis_result = imaging(
                window_title=app_name, additional_context=element_analysis, x=int(x), y=int(y)
            )
            print(element_analysis_result)

            # Check if the result is None or doesn't contain the necessary data
            if (
                element_analysis_result is None
                or "choices" not in element_analysis_result
                or len(element_analysis_result["choices"]) == 0
                or "message" not in element_analysis_result["choices"][0]
                or "content" not in element_analysis_result["choices"][0]["message"]
            ):
        ...
        coordinates, selected_element, keywords, attempt = find_element(
            single_step, app_name, original_goal, assistant_goal, attempt=0
        )
        x = coordinates["x"]
        y = coordinates["y"]
        print(f"Coordinates: {x} and {y}")
...
def fast_act(
    ...
    if visioning_context:
        ...
        speaker("Clicking onto the element without visioning context.")
        generate_keywords = [
            {
                "role": "assistant",
                "content": "You are an AI Agent called keyword Element Generator that receives the description "
...
def control_mouse(generated_coordinates, double_click=None, goal=""):
    ...
    coordinates = {k.strip(): int(v.strip()) for k, v in (item.split("=") for item in generated_coordinates.split(","))}
    x = coordinates["x"]
    y = coordinates["y"]
    pyautogui.moveTo(x, y, 0.5, pyautogui.easeOutQuad)
    pyautogui.click(x, y)
...
def write_action(goal=None, assistant_identity="", press_enter=False, app_name="", original_goal=None, last_step=""):
    ...
    message_writer_agent = [
        {
            "role": "assistant",
            "content": f"You're an AI Agent called Writter that processes the goal and only returns the final text goal.{assistant_identity_msg}\n"
    ...
    if (
        "press enter" in goal.lower()
        or "press the enter" in goal.lower()
        or "'enter'" in goal.lower()
        or '"enter"' in goal.lower()
        or press_enter is True
    ):
...
def perform_simulated_keypress(press_key):
    ...
    keys_pattern = (
        r"\b(Win(?:dows)?|Ctrl|Alt|Shift|Enter|Space(?:\s*Bar)?|Tab|Esc(?:ape)?|Backspace|Insert|Delete|"
...
if __name__ == "__main__":
    assistant(
        assistant_goal="Open Reddit, Youtube, TikTok, and Netflix on new windows by using the keyboard on each corner of the screen.",
        app_name="firefox",
        execute_json_case=json_case_example,
    )
    assistant(
        assistant_goal="Open a new tab the song 'Wall Of Eyes - The Smile', from google search results filter by videos then play it on Firefox"
    )
